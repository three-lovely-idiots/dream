...
function  getTree2($data,$pid,$deep=0)
{
    //static $tree=array();
    static $tree,$count;
    foreach ($data as $row) {
        if($row['pid']==$pid){
             $row['deep']=$deep;
             $row['parent'] = isset($tree[$deep-1]) ? $tree[$deep-1]['name'] : '顶级分类';
             $tree[] = $row;
             getTree2($data,$row['id'],$deep+1);
        }
    }
    return $tree;
}
...
//以上和这个两种树形排序
//
//function  getMenu($data,$pid,$deep=0)
//{
//    //static $tree=array();
//    $tree='';
//    foreach ($data as $row) {
//        if($row['pid']==$pid){
//            $row['deep']=$deep;
//            $row['pid']=getMenu($data,$row['id'],$deep+1);
//            $tree[]=$row;
