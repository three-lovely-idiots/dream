    ...
    function flowLoad($category_id){
        ...
        flow.load({
            elem: '#Images', //流加载容器
                ...
                $.post("mySelect", {"type_id":$category_id},function(res){
                    //插入
                    var imgList = [],data = res;
                    var maxPage = imgNums*page < data.length ? imgNums*page : data.length;
                    setTimeout(function(){
                        for(var i=imgNums*(page-1); i<maxPage; i++){
                            imgList.push('<li><img layer-src="'+data[i].topic_img.url+'"  src="'+data[i].topic_img.url+'"  alt="'+data[i].id+'"><div class="operate"><div class="check"><input type="radio" name="img" value="'+data[i].id+'" lay-filter="choose" lay-skin="primary" title="'+data[i].name+'"></div><i class="layui-icon img_del">&#xe640;</i></div></li>');
                        }
